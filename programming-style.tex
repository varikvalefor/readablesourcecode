...
The janky-looking defun which appears multiple times in \texttt{divisors} can become a tradfun, yielding the following result:
\begin{lstlisting}
⎕IO←0
i ← {1+⍳⍵}
divides ← {0=⍺|⍵}
...
factor ← {1 = ⍵ : 1 ⋄ isPrime ⍵ : ⍵ ⋄ ⊃,/factor ¨ ⍵ {⍵,⍺÷⍵} ⌊/divisors ⍵}
\end{lstlisting}
Adding comments to this snippet is left as an exercise for the reader.
